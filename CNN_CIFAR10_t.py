...
def convolutional_neural_network(x):#, keep_rate):
    weights = {
        # 5 x 5 convolution, 1 input image, 32 outputs
        'W_conv1': tf.Variable(tf.random_normal([3, 3, image_channels , 32],stddev=0.01),name='conv1_w'),
        'W_conv2': tf.Variable(tf.random_normal([3, 3, 32 , 32],stddev=0.01),name='conv2_w'),
        # 5x5 conv, 32 inputs, 64 outputs 
        'W_conv3': tf.Variable(tf.random_normal([3, 3, 32, 64],stddev=0.01),name='conv3_w'),
        'W_conv4': tf.Variable(tf.random_normal([3, 3, 64, 64],stddev=0.01),name='conv4_w'),
        'W_conv5': tf.Variable(tf.random_normal([3, 3, 64, 64],stddev=0.01),name='conv5_w'),
        # fully connected, 8*8*64 inputs, 1024 outputs
        'W_fc1': tf.Variable(tf.random_normal([4*4*64, 512],stddev=0.01),name='fc1_w'),
        'W_fc2': tf.Variable(tf.random_normal([512, 256],stddev=0.01),name='fc2_w'),
        'W_fc3': tf.Variable(tf.random_normal([256, 256],stddev=0.01),name='fc3_w'),
        # 1024 inputs, 10 outputs (class prediction)
        'out': tf.Variable(tf.random_normal([256, n_classes],stddev=0.01),name='out_w')
    }
    
    biases = {
        'b_conv1': tf.Variable(tf.ones([32]),name='conv1_b'),
        'b_conv2': tf.Variable(tf.ones([32]),name='conv2_b'),
        'b_conv3': tf.Variable(tf.ones([64]),name='conv3_b'),
        ...
        'b_fc2': tf.Variable(tf.ones([256]),name='fc2_b'),
        'b_fc3': tf.Variable(tf.ones([256]),name='fc3_b'),
    ...
    conv3 = tf.nn.relu(conv2d(conv3, weights['W_conv4'],name='CONV4') + biases['b_conv4'])
    conv3 = tf.nn.relu(conv2d(conv3, weights['W_conv5'],name='CONV5') + biases['b_conv5'])
    # Max Pooling (down-sampling)
    conv3 = maxpool2d(conv3, name='MAX_POOL3')
    conv3 = tf.nn.dropout(conv3, keep_prob, name='DROPOUT')
    
    # Fully connected layer
    # Reshape conv2 output to fit fully connected layer
    fc = tf.reshape(conv3, [-1, 4*4*64])

    fc = tf.nn.relu(tf.matmul(fc, weights['W_fc1'],name='FC1') + biases['b_fc1'])
    fc = tf.nn.dropout(fc, keep_prob, name='DROPOUT')
    fc = tf.nn.relu(tf.matmul(fc, weights['W_fc2'],name='FC2') + biases['b_fc2'])
    fc = tf.nn.dropout(fc, keep_prob, name='DROPOUT')
    fc = tf.nn.relu(tf.matmul(fc, weights['W_fc3'],name='FC3') + biases['b_fc3'])
...
#            dev = readCIFAR.DataSet(Xtr[:32000], dev_labels)
            ...
            for _ in range(int(train.num_examples/batch_size)):
                ...
                _, c = sess.run([optimizer, cost], feed_dict={x: epoch_x, y: epoch_y ,keep_prob: 0.6})
                epoch_loss += c
...
file_path = "/home/ti1080/GP_TEAM2_salama/model2_try/model_2.ckpt"
weights_path = "/home/ti1080/GP_TEAM2_salama/model2_try/model_2_weights.ckpt"
biases_path = "/home/ti1080/GP_TEAM2_salama/model2_try/model_2_biases.ckpt"
