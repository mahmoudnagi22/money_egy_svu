...
y_pred = tf.nn.softmax(layer_fc2,name='OUT')
...
optimizer = tf.train.GradientDescentOptimizer(learning_rate=5e-2).minimize(cost)
...
with tf.Session() as session:
    ...
    tf.train.write_graph(session.graph_def, '.', 'tfdroid.pbtxt') 
    ...
    num_iterations=50*int(data.train.num_examples/batch_size)
    ...
    for i in range(total_iterations,
        ...
        if i % int(data.train.num_examples/batch_size) == 0: 
            ...
            saver.save(session, 'my_test_model')
            per=time.time()-b
            print("epoch period :",per)
            print_progress(epoch, feed_dict_train, feed_dict_validate, val_loss)
            b = time.time()
        #print("d3 =",time.time()-be)
    # Update the total number of iterations performed.
    total_iterations += num_iterations
    saver1.save(session, 'tfdroid.ckpt')
